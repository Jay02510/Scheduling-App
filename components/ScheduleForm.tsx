...
const ScheduleForm: React.FC<ScheduleFormProps> = ({ 
  ...
  const addSubjectToClass = (classId: string) => {
    const name = prompt("Subject Name (e.g., Mathematics, Robotics):");
    if (!name) return;
    ...
    if (!subject) {
      const newId = `sub-${Math.random().toString(36).substr(2, 5)}`;
      subject = { id: newId, name, frequencyPerWeek: 5, gradeLevels: ['G1'] };
    ...
    setClasses(classes.map(c => {
      if (c.id === classId) {
        ...
        if (c.assignments.find(a => a.subjectId === subject!.id)) return c;
        return { ...c, assignments: [...c.assignments, { subjectId: subject!.id, teacherId: '' }] };
      }
      return c;
    }));
  };
    ...
    if (detailView.type === 'class') {
      ...
      return (
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="bg-white p-10 rounded-[3rem] border shadow-2xl max-w-3xl mx-auto space-y-10">
            <div className="flex justify-between items-center border-b pb-8">
               ...
               <div className="flex items-center gap-4">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Lead:</span>
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Subject Curriculum</h4>
                <button onClick={() => addSubjectToClass(cls.id)} className="px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl font-black text-[9px] uppercase tracking-widest hover:bg-indigo-100 transition-all">+ Add Subject</button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[400px] overflow-y-auto custom-scrollbar pr-2">
                {cls.assignments.length > 0 ? cls.assignments.map(a => (
                  <div key={a.subjectId} className="p-6 bg-slate-50 rounded-[1.8rem] border border-slate-100 space-y-4">
                    ...
                    <div className="space-y-2">
                       <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block ml-1">Assigned Staff</span>
                       <select className="w-full bg-white border border-slate-100 rounded-xl px-4 py-2 font-black text-indigo-500 text-[10px] uppercase outline-none" value={a.teacherId} onChange={e => {
                  ...
                  <div className="col-span-full py-16 text-center bg-slate-50 rounded-[2.5rem] border-2 border-dashed border-slate-100">
                    <p className="text-[10px] font-black text-slate-300 uppercase italic">No curriculum mapped to this group</p>
                  </div>
                )}
              </div>
            </div>

            <button onClick={() => { setClasses(classes.filter(c => c.id !== cls.id)); setDetailView(null); }} className="w-full py-5 bg-rose-50 text-rose-500 rounded-2xl font-black text-[10px] uppercase tracking-widest border border-rose-100">Delete Class Group</button>
          </div>
        </div>
      );
    }
      ...
      <div className="bg-white p-6 md:p-10 rounded-[2.5rem] shadow-2xl border border-slate-50 min-h-[500px]">
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="mt-4 flex gap-2">
                       <span className="text-[7px] font-black text-indigo-500 bg-indigo-50 px-2 py-1 rounded-md uppercase tracking-widest">{c.assignments.length} Subjects</span>
                    </div>
                  </div>
        ...
        {activeTab === 'subjects' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-fadeIn">
              ...
              return (
                <div key={s.id} className="p-6 bg-slate-50 rounded-[2rem] border border-transparent hover:border-slate-200 transition-all overflow-hidden flex flex-col gap-4">
                  <div className="flex items-center justify-between gap-4">
                    <input className="bg-white border rounded-xl px-4 py-2.5 w-full font-black text-slate-900 uppercase text-xs outline-none" value={s.name} onChange={e => setSubjects(subjects.map(sub => sub.id === s.id ? {...sub, name: e.target.value} : sub))} />
                  ...
                  <button 
                    ...
                    className={`w-full py-3 rounded-xl border text-[9px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${isExpanded ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg' : 'bg-white border-slate-100 text-slate-400 hover:bg-slate-50'}`}
                  >
                    <span>{isExpanded ? 'Collapse Faculty Mapping' : 'Manage Faculty Assignments'}</span>
                  ...
                  {isExpanded && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 animate-fadeIn">
                      {classes.map(c => (
                        <div key={c.id} className="bg-white p-4 rounded-[1.2rem] border border-slate-100 flex flex-col gap-2">
                          ...
                          <select className="w-full bg-slate-50 border border-transparent rounded-lg px-3 py-1.5 font-bold text-slate-900 text-[10px] outline-none hover:border-slate-200 transition-all" value={c.assignments.find(a => a.subjectId === s.id)?.teacherId || ''} onChange={e => {
            ...
            <button onClick={handleAddNewSubjectGlobal} className="p-6 border-2 border-dashed border-slate-200 rounded-[2rem] text-slate-300 font-black text-[10px] uppercase tracking-widest hover:border-indigo-300 transition-all">+ New Lesson Block</button>
          </div>
        )}
