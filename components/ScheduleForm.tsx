    ...
    <div className="space-y-8 animate-fadeIn">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm gap-6">
        <div>
          ...
          <p className="text-slate-500 font-bold text-[10px] uppercase tracking-widest mt-1">Manage Faculty, Classes & Subjects</p>
        </div>
        ...
        {activeTab === 'staff' && (
          <div className="space-y-10">
            <div className="bg-[#0f172a] p-8 rounded-[2rem] shadow-2xl relative overflow-hidden group">
               ...
               <h4 className="text-indigo-400 font-black text-[10px] uppercase tracking-[0.2em] mb-4">Add Multiple Teachers</h4>
               <textarea 
                ...
                placeholder="Paste list (e.g., Mr. Smith - Math, Ms. Jones - Grade 1 Homeroom...)"
               ...
               >
                 {isProcessingPaste ? 'Parsing...' : 'Import List'}
               </button>
            </div>
                ...
                return (
                  <div key={t.id} className="p-8 bg-slate-50 rounded-[2.5rem] border border-transparent hover:border-slate-200 transition-all space-y-6 relative group">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${isOverloaded ? 'bg-rose-100 text-rose-600' : 'bg-emerald-100 text-emerald-600'}`}>
                            Weekly Load: {weeklyLoad} / {capacity} Classes
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                       ...
                       <div className="space-y-1">
                          <label className="text-[8px] font-black text-slate-400 uppercase tracking-widest ml-1">Breaks Needed</label>
          ...
          <div className="space-y-10">
            <div className="flex justify-between items-center bg-indigo-50 p-8 rounded-[2.5rem] border border-indigo-100 shadow-sm">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="text-indigo-900 font-black text-sm uppercase">Class Assignments</h4>
                  <p className="text-indigo-500 text-[9px] font-bold uppercase tracking-widest">Connect Teachers to Class Subjects</p>
                </div>
              </div>
              <button 
              ...
              >
                {isAutoMapping ? 'Mapping...' : 'Auto-Assign Teachers'}
              </button>
            </div>

            <div className="space-y-12">
              {classes.map(c => (
                <div key={c.id} className="bg-slate-50 p-10 rounded-[3rem] border border-transparent hover:border-slate-200 transition-all space-y-8 relative group">
                  <button onClick={() => setClasses(classes.filter(pc => pc.id !== c.id))} className="absolute top-10 right-10 text-slate-300 hover:text-rose-500 transition-colors opacity-0 group-hover:opacity-100">
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M6 18L18 6M6 6l12 12" /></svg>
                  </button>

                  <div className="flex items-center gap-5">
                    <div className="w-10 h-10 rounded-xl shadow-lg" style={{ backgroundColor: c.color }}></div>
                    <input className="bg-transparent border-0 p-0 font-black text-slate-800 text-2xl w-full focus:ring-0 uppercase tracking-tight" value={c.name} onChange={e => setClasses(classes.map(pc => pc.id === c.id ? {...pc, name: e.target.value} : pc))} />
                  </div>
                        ...
                        <div key={s.id} className={`p-6 rounded-[2rem] border-2 transition-all ${isAssigned ? 'bg-white border-indigo-100 shadow-md scale-[1.02]' : 'bg-slate-100/50 border-transparent opacity-60'}`}>
                          <div className="flex justify-between items-start mb-4">
                            <div>
                               ...
                               <p className="text-[8px] font-bold text-slate-400 uppercase tracking-tighter mt-0.5">{s.frequencyPerWeek} Times / Week</p>
                            </div>
                            <input 
                              type="checkbox" 
                              checked={isAssigned} 
              ...
              <button onClick={addNewClass} className="w-full py-12 border-4 border-dashed border-slate-100 rounded-[3rem] text-slate-300 font-black text-[12px] uppercase tracking-widest hover:border-indigo-100 hover:text-indigo-500 transition-all flex items-center justify-center gap-3">
                 + Create New Class
              </button>
            </div>
          </div>
        ...
        {activeTab === 'subjects' && (
          <div className="space-y-10">
            <div className="flex justify-between items-center">
               <h4 className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Global Subject List</h4>
               <button onClick={addNewSubject} className="bg-slate-900 text-white px-8 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:scale-105 transition-all shadow-lg">+ Add Subject</button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {subjects.map(s => (
                <div key={s.id} className="p-6 bg-slate-50 border border-slate-100 rounded-[2rem] flex flex-col gap-4 relative group">
                  <button onClick={() => setSubjects(subjects.filter(ps => ps.id !== s.id))} className="absolute top-6 right-6 text-slate-300 hover:text-rose-500 transition-colors opacity-0 group-hover:opacity-100">
                  ...
                  <div className="space-y-1">
                    <label className="text-[8px] font-black text-slate-400 uppercase">Times per week</label>
                    <input type="number" className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 font-bold outline-none" value={s.frequencyPerWeek} onChange={e => setSubjects(subjects.map(ps => ps.id === s.id ? {...ps, frequencyPerWeek: parseInt(e.target.value) || 0} : ps))} />
                  </div>
                </div>
              ))}
            </div>
        ...
        {activeTab === 'locks' && (
          <div className="space-y-10">
             <div className="bg-indigo-50 p-8 rounded-[2.5rem] border border-indigo-100 text-center">
                <p className="text-indigo-900 font-black text-[11px] uppercase tracking-widest mb-2">School-wide Lock Management</p>
                <p className="text-indigo-400 text-[9px] font-bold uppercase tracking-tight">Configure Lunch Times, Assemblies and Holidays for the whole school.</p>
             </div>
             <div className="py-20 text-center bg-slate-50 rounded-[2.5rem] border-2 border-dashed border-slate-200">
                <p className="text-slate-400 font-black text-[10px] uppercase tracking-widest">School Calendar & Lock Controls</p>
                <button onClick={() => setFixedClasses([])} className="mt-4 text-[9px] font-black uppercase text-indigo-500">Reset All Locks</button>
             </div>
          </div>
        )}
      </div>
