...
const ScheduleForm: React.FC<ScheduleFormProps> = ({ 
  onGenerate, profile, setProfile, teachers, setTeachers, classes, setClasses, textbooks, subjects, setSubjects, lockedSlots, setLockedSlots, schedule, onMoveLock, onFillLocks, onNavigate, language 
}) => {
  ...
  const addTemplateToPrompt = (text: string) => {
    if (!profile) return;
  ...
  const updateSubjectFrequency = (subjectId: string, delta: number) => {
    setSubjects(subjects.map(s => s.id === subjectId ? { ...s, frequencyPerWeek: Math.max(1, s.frequencyPerWeek + delta) } : s));
  };
  ...
  if (detailView) {
    if (detailView.type === 'teacher') {
      ...
      return (
        <div className="space-y-6 animate-fadeIn">
          <button onClick={() => setDetailView(null)} className="flex items-center gap-2 text-[10px] font-black uppercase text-slate-400 hover:text-indigo-500 transition-colors">← {language === 'ko' ? '목록으로' : 'Back to Staffing'}</button>
          <div className="bg-white p-10 rounded-[3rem] border shadow-2xl max-w-2xl mx-auto space-y-10">
            ...
            <div className="grid grid-cols-2 gap-8">
              <div className="space-y-3">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block ml-1">{language === 'ko' ? '할당된 역할' : 'Assigned Role'}</span>
              ...
              <div className="space-y-3">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block ml-1">{language === 'ko' ? '일일 최대 교시' : 'Max Daily Periods'}</span>
    ...
    if (detailView.type === 'class') {
      ...
      return (
        <div className="space-y-6 animate-fadeIn">
          <button onClick={() => setDetailView(null)} className="flex items-center gap-2 text-[10px] font-black uppercase text-slate-400 hover:text-indigo-500 transition-colors">← {language === 'ko' ? '목록으로' : 'Back to Classes'}</button>
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-center bg-slate-50 p-6 rounded-[2rem] border border-slate-100">
                <div className="flex-1 flex gap-3">
                  <input type="text" placeholder="Subject Name (e.g. Science, Art)..." className="flex-1 bg-white border border-slate-200 rounded-xl px-6 py-3 font-bold text-xs outline-none focus:border-indigo-500" value={newSubjectName} onChange={(e) => setNewSubjectName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && addSubjectToClassDetail(cls.id)} />
                  <button onClick={() => addSubjectToClassDetail(cls.id)} className="px-8 py-3 bg-[#0f172a] text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-xl hover:scale-105 active:scale-95 transition-all">+ Add Subject</button>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[400px] overflow-y-auto custom-scrollbar pr-2">
                        ...
                        <div className="flex flex-col">
                          ...
                          <div className="flex items-center gap-3 mt-1.5">
                            ...
                            <div className="flex items-center gap-1.5 bg-slate-50 px-2 py-1 rounded-lg border border-slate-100">
                              <button onClick={(e) => { e.stopPropagation(); updateSubjectFrequency(a.subjectId, -1); }} className="w-4 h-4 flex items-center justify-center font-black text-slate-300 hover:text-indigo-600 transition-colors">-</button>
                              <span className="text-[10px] font-black text-indigo-600 w-3 text-center">{subject?.frequencyPerWeek || 5}</span>
                              <button onClick={(e) => { e.stopPropagation(); updateSubjectFrequency(a.subjectId, 1); }} className="w-4 h-4 flex items-center justify-center font-black text-slate-300 hover:text-indigo-600 transition-colors">+</button>
                            </div>
                            <span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">{t('freq_per_week')}</span>
                          </div>
                        </div>
                        <button onClick={() => setClasses(classes.map(c => c.id === cls.id ? { ...c, assignments: c.assignments.filter(as => as.subjectId !== a.subjectId) } : c))} className="text-slate-300 hover:text-rose-500 transition-colors p-1">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M6 18L18 6M6 6l12 12" /></svg>
                        </button>
                      </div>
                      <div className="space-y-1.5">
                         <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block ml-1">{t('mapping_staff')}</span>
                ...
                }) : <div className="col-span-full py-16 text-center bg-slate-50 rounded-[2.5rem] border-2 border-dashed border-slate-100"><p className="text-[10px] font-black text-slate-300 uppercase italic">Start by adding subjects to this group's curriculum</p></div>}
              </div>
            </div>
            <button onClick={() => { setClasses(classes.filter(c => c.id !== cls.id)); setDetailView(null); }} className="w-full py-5 bg-rose-50 text-rose-500 rounded-2xl font-black text-[10px] uppercase tracking-widest border border-rose-100">{t('delete_group')}</button>
          </div>
        </div>
      );
    }
    if (detailView.type === 'lock') {
      ...
      return (
        <div className="space-y-6 animate-fadeIn">
          <button onClick={() => setDetailView(null)} className="flex items-center gap-2 text-[10px] font-black uppercase text-slate-400 hover:text-indigo-500 transition-colors">← {language === 'ko' ? '목록으로' : 'Back to Grid'}</button>
          <div className="bg-white p-10 rounded-[3rem] border shadow-2xl max-w-xl mx-auto space-y-8">
            ...
            <label className="flex items-center justify-between p-6 bg-slate-900 rounded-[1.8rem] cursor-pointer text-white">
              <div><span className="text-[10px] font-black uppercase tracking-widest block">Institutional Event</span><span className="text-[8px] text-slate-400 font-bold uppercase">Blocks every class group at this time</span></div>
  ...
  const tuningTemplates = [
    { title: "Teacher Schedule", prompt: "Teacher [NAME] can only teach in the morning periods.", icon: "👤" },
    { title: "Subject Balance", prompt: "Priority for Mathematics and English in the first 3 periods.", icon: "📚" },
    { title: "Planning Blocks", prompt: "Ensure all Homeroom teachers have a joint planning block on Friday afternoons.", icon: "🗓️" },
    { title: "Room Limits", prompt: "The Art room is only available on Tue/Wed.", icon: "🎨" }
  ];
  ...
  return (
    <div className="space-y-8 animate-fadeIn pb-16" onMouseUp={onFillEnd}>
      <header className="flex flex-col md:flex-row justify-between items-center bg-white p-8 rounded-[3rem] border shadow-sm gap-6 relative overflow-hidden">
        ...
        <div>
          <h2 className="text-3xl font-black text-slate-900 uppercase tracking-tighter">{t('setup')}</h2>
          <p className="text-slate-400 text-[10px] font-bold uppercase tracking-[0.3em] mt-1">Institutional Configuration</p>
        </div>
        <div className="flex gap-3">
          {schedule && <button onClick={() => onNavigate?.('homerooms')} className="bg-white border border-slate-200 text-slate-900 px-6 py-4 rounded-[1.5rem] font-black text-[10px] uppercase tracking-widest hover:bg-slate-50 transition-all">{t('homeroom_portal')}</button>}
          <button onClick={onGenerate} className="gradient-primary text-white px-8 py-4 rounded-[1.5rem] shadow-xl font-black text-[10px] uppercase tracking-widest hover:scale-105 active:scale-95 transition-all">{t('sync_infra')}</button>
        </div>
      </header>
      <div className="flex bg-slate-100 p-1.5 rounded-[1.5rem] w-fit shadow-inner mx-auto md:mx-0 overflow-x-auto max-w-full gap-1">
        {[
          ...
          { id: 'global', label: t('global_engagements') },
          { id: 'rhythm', label: t('daily_rhythm') },
          { id: 'tuning', label: t('ai_tuning') }
        ].map(t => (
              ...
              <div key={c.id} onClick={() => setDetailView({ type: 'class', id: c.id })} className="p-8 bg-slate-50 rounded-[2rem] cursor-pointer hover:shadow-xl transition-all border-l-[10px] group border-transparent hover:border-slate-100" style={{ borderLeftColor: c.color }}>
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="mt-4 flex gap-2"><span className="text-[7px] font-black text-indigo-500 bg-indigo-50 px-2 py-1 rounded-md uppercase tracking-widest">{(c.assignments || []).length} Subject Blocks</span></div>
                  </div>
              ...
              return (
                <div key={s.id} className="p-6 bg-slate-50 rounded-[2rem] border border-transparent hover:border-slate-200 transition-all overflow-hidden flex flex-col gap-4">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <button onClick={() => setSubjects(subjects.filter(sub => sub.id !== s.id))} className="p-2 text-slate-200 hover:text-rose-500 transition-colors"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                  </div>
                  <button onClick={() => setExpandedSubjectId(isExpanded ? null : s.id)} className={`w-full py-3.5 rounded-xl border text-[9px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${isExpanded ? 'bg-[#0f172a] border-[#0f172a] text-white shadow-xl' : 'bg-white border-slate-100 text-slate-400 hover:bg-slate-50'}`}><span>{isExpanded ? 'Collapse Faculty Mapping' : 'Manage Staff Assignments'}</span><svg className={`w-3 h-3 transition-transform ${isExpanded ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 9l-7 7-7-7" /></svg></button>
                  {isExpanded && (
                      ...
                      {classes.map(c => {
                        ...
                        return (
                          <div key={c.id} className={`bg-white p-4 rounded-[1.2rem] border flex flex-col gap-2 transition-all ${assignment ? 'border-indigo-100 shadow-sm' : 'border-slate-50 opacity-60 hover:opacity-100'}`}>
                            <div className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: c.color }}></div><span className="text-[8px] font-black text-slate-500 uppercase tracking-tighter">{c.name}</span></div>
                            ...
                            }}><option value="">Vacant</option>{teachers.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}</select>
                          </div>
                        );
                      })}
                    </div>
        ...
        {activeTab === 'global' && (
          <div className="space-y-6 animate-fadeIn">
            <div className="bg-[#0f172a] p-6 rounded-[2rem] text-white flex justify-between items-center">
              <div><p className="text-[10px] font-black uppercase tracking-[0.2em]">Institutional Engagements</p><p className="text-slate-500 text-[8px] font-bold uppercase tracking-widest mt-1">Global & Selective Infrastructure Blocks</p></div>
              <div className="flex gap-4"><span className="text-[8px] font-black text-indigo-400 bg-indigo-500/10 px-3 py-1.5 rounded-lg border border-indigo-500/20 uppercase tracking-widest">Swap/Clone/Fill Active</span></div>
            </div>
                      ...
                      return (
                        <td key={dIdx} onMouseEnter={() => onFillMove(dIdx, pIdx)} className="h-[110px] relative transition-all">
                          <button onClick={() => openLockConfig(dIdx, pIdx)} draggable={!!lock} onDragStart={() => onLockDragStart(dIdx, pIdx)} onDragOver={(e) => onLockDragOver(e, dIdx, pIdx)} onDrop={(e) => onLockDrop(e, dIdx, pIdx)} className={`group w-full h-full rounded-[1.8rem] border-[2px] flex flex-col items-center justify-center p-3 transition-all relative overflow-hidden ${lock ? 'bg-white border-slate-900 shadow-md z-10' : 'bg-white border-slate-100 hover:border-indigo-100 hover:shadow-sm'}`}>
                            {lock ? (<><span className="text-[12px] font-black text-slate-900 uppercase text-center leading-tight truncate px-1 tracking-tighter">{lock.name}</span>{lock.isSchoolWide ? <span className="text-[7px] font-black text-indigo-500 uppercase tracking-widest mt-1 opacity-60">Global Block</span> : <div className="flex flex-wrap justify-center gap-0.5 mt-1.5 px-2">{(lock.classIds || []).map(cid => <div key={cid} className="px-1.5 py-0.5 rounded-[4px] text-[6px] font-black text-white uppercase shadow-sm" style={{ backgroundColor: getClassColor(cid) }}>{getClassName(cid)}</div>)}</div>}<div onMouseDown={(e) => onFillStart(e, dIdx, pIdx)} className="absolute bottom-1.5 right-1.5 w-3 h-3 bg-indigo-600 rounded-md border border-white cursor-crosshair opacity-0 group-hover:opacity-100 transition-opacity"></div></>) : <div className="text-slate-200 text-xl font-black">+</div>}
