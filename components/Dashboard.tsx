...
const Dashboard: React.FC<DashboardProps> = ({ teachers, classes, textbooks, onResync }) => {
  const stats = [
    ...
    { name: 'Health', value: 94, unit: '%', color: '#2dd4bf', icon: 'M13 10V3L4 14h7v7l9-11h-7z' },
  ];
  ...
  return (
    <div className="space-y-10 animate-fadeIn max-w-full overflow-hidden">
      <header className="px-1 flex justify-between items-end">
        <div>
          <h2 className="text-4xl font-black text-slate-900 tracking-tight">School Dashboard</h2>
          <p className="text-slate-500 font-bold text-xs uppercase tracking-widest mt-1">Live School Summary</p>
        </div>
        <div className="hidden md:block">
           <span className="bg-emerald-50 text-emerald-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest">System Active</span>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
        <div className="xl:col-span-8 bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100">
          <div className="flex justify-between items-center mb-10">
            <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Class & Teacher Workload</h3>
            <div className="flex gap-2">
               <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-indigo-500"></div><span className="text-[8px] font-bold text-slate-400 uppercase">Load</span></div>
               <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-slate-200"></div><span className="text-[8px] font-bold text-slate-400 uppercase">Limit</span></div>
            </div>
          </div>
        ...
        <div className="xl:col-span-4 space-y-6">
          <div className="bg-[#0f172a] p-10 rounded-[3rem] shadow-xl text-white relative overflow-hidden group">
            ...
            <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-400 mb-6">School Summary</h3>
            <div className="space-y-4">
              <div className="flex items-center justify-between border-b border-white/10 pb-4">
                <span className="text-xs font-bold text-slate-400">Schedule Health</span>
                <span className="text-sm font-black text-emerald-400">98% Perfect</span>
              </div>
              <div className="flex items-center justify-between border-b border-white/10 pb-4">
                <span className="text-xs font-bold text-slate-400">Active Conflicts</span>
                <span className="text-sm font-black text-white">0 Issues</span>
              </div>
            </div>
            <button onClick={onResync} className="w-full mt-10 py-4 bg-white/10 hover:bg-white/20 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all">Adjust Setup</button>
          </div>

          <div className="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100">
             <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-6">Staff Wellness</h3>
             <div className="flex items-center gap-4">
                <div className="flex-1 space-y-1">
                   ...
                   <div className="flex justify-between text-[8px] font-black uppercase tracking-tighter text-slate-400">
                      ...
                      <span className="text-indigo-600">Optimal (85% safe)</span>
                   </div>
                </div>
             </div>
          </div>
