...
const ResourcePlanner: React.FC<ResourcePlannerProps> = ({ textbooks, onUpdate, profile, classes }) => {
  ...
  const handleDelete = (id: string) => {
    if (window.confirm("Delete this textbook?")) {
  ...
  return (
    <div className="space-y-10 animate-fadeIn max-w-full overflow-hidden">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          <h2 className="text-3xl font-black text-slate-900 tracking-tight uppercase">Curriculum Books</h2>
          <p className="text-slate-500 font-bold text-[10px] uppercase tracking-widest mt-1">Class-Specific Learning Resources</p>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 gap-6">
        {filteredTextbooks.length === 0 ? (
          <div className="py-20 text-center bg-white rounded-[2.5rem] border-2 border-dashed border-slate-100">
             <p className="text-slate-300 font-black text-[10px] uppercase tracking-widest">No books assigned here</p>
          </div>
        ) : (
              ...
              <div className="flex flex-col lg:flex-row gap-8 items-start">
                ...
                <div className="flex-1 grid grid-cols-2 sm:grid-cols-4 gap-6 w-full items-center">
                  <div className="space-y-1">
                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block ml-1">Chapters</span>
                    <input 
                      type="number"
                  ...
                  <div className="space-y-1">
                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block ml-1">Pages</span>
                    <input 
                      type="number"
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 font-bold text-slate-900 outline-none"
                  ...
                  <div className="space-y-1">
                    <span className="text-[9px] font-black text-emerald-600 uppercase tracking-widest block ml-1">Current</span>
                    <input 
                      type="number"
                      className="w-full bg-emerald-50 border border-emerald-100 rounded-xl px-4 py-2.5 font-bold text-emerald-900 outline-none"
                  ...
                  <div className="flex items-center justify-end gap-4">
                    <button 
                      ...
                      className="p-3 bg-rose-50 text-rose-500 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity"
                    >
        ...
        >
          <div className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center text-slate-300 transition-colors">
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M12 4v16m8-8H4" /></svg>
          </div>
          <span>Assign New Book to {activeClassId === 'general' ? 'General Pool' : classes.find(c => c.id === activeClassId)?.name}</span>
        </button>
      </div>
    </div>
  );
