...
const Onboarding: React.FC<OnboardingProps> = ({ onComplete }) => {
  ...
  const addClass = () => {
    ...
    setProfile({...profile, classes: [...profile.classes, { id, name: `Group ${profile.classes.length + 1}`, grade: 'G1', homeroomTeacherId: '', assignments: [], color: CLASS_COLORS[profile.classes.length % CLASS_COLORS.length] }]});
  };

  const addTeacher = () => {
    const id = Math.random().toString(36).substr(2, 9);
    setProfile({...profile, teachers: [...profile.teachers, { id, name: 'New Faculty', role: 'subject', subjects: [], maxDailyPeriods: 8, breaksNeededPerWeek: 5, color: TEACHER_COLORS[profile.teachers.length % TEACHER_COLORS.length], assignedClasses: [], employmentType: 'Full-Time' }]});
  };
          ...
          <div className="space-y-6">
            {[1, 2, 3, 4, 5].map(s => (
              <div key={s} className={`flex items-center gap-4 transition-all ${step === s ? 'translate-x-1 opacity-100' : 'opacity-30'}`}>
                ...
                <span className="text-[9px] font-black uppercase tracking-widest">{['Identity', 'Groupings', 'Faculty', 'Locks', 'Lessons'][s-1]}</span>
              </div>
            ))}
          </div>
        </div>
              ...
              <div className="space-y-8 animate-fadeIn">
                ...
                <div className="space-y-6">
                  <label className="block">
                    ...
                    <input className="w-full bg-white border border-slate-200 rounded-2xl px-6 py-4 font-bold text-slate-900 outline-none focus:ring-4 focus:ring-indigo-500/10 transition-all" value={profile.name} onChange={e => setProfile({...profile, name: e.target.value})} placeholder="Westview Primary" />
                  </label>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-slate-900 p-6 rounded-[2rem] text-white">
                      <span className="text-[9px] font-black text-slate-500 uppercase block mb-2">Daily Slots</span>
            ...
            {step === 2 && (
              <div className="space-y-8 animate-fadeIn">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <button onClick={addClass} className="p-6 border-2 border-dashed border-indigo-100 rounded-3xl text-indigo-400 font-black text-[10px] uppercase">+ Add Group</button>
                </div>
              </div>
            )}
            ...
            {step === 3 && (
              <div className="space-y-8 animate-fadeIn">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <button onClick={addTeacher} className="p-8 border-2 border-dashed border-slate-100 rounded-[2rem] text-slate-300 font-black text-[10px] uppercase">+ Faculty</button>
                </div>
              </div>
            )}
