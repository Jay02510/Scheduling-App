...
const ScheduleViewer: React.FC<ScheduleViewerProps> = ({ schedule, classes, teachers, subjects, textbooks, lockedSlots, profile, onGenerateRoadmap, onUpdateSlot, onMoveSlot, onFillSlots, onNavigate, onJump, onRegenerate, initialClassId, language }) => {
  ...
  return (
    <div className="space-y-8 animate-fadeIn max-w-full pb-32 relative" id="homeroom-schedule">
      ...
      {errorToast && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 z-[300] flex items-center gap-4 bg-[#0f172a] text-white px-8 py-4 rounded-[1.8rem] shadow-[0_0_0_1px_rgba(244,63,94,0.3),0_20px_40px_rgba(0,0,0,0.6)] border border-slate-800 toast-enter">
          ...
          <div className="flex flex-col">
            <span className="text-xs font-black uppercase tracking-[0.2em] text-rose-400">Logic Violation</span>
      ...
      <header className="flex flex-col md:flex-row justify-between items-end gap-6 no-print">
        <div>
          <h2 className="text-4xl font-black text-white tracking-tight">{t('homeroom_portal')}</h2>
          <div className="flex items-center gap-3 mt-2">
            <span className="text-slate-500 font-bold text-xs tracking-[0.06em]">{t('institution_grid')} • {currentClass.name}</span>
            <div className="px-3 py-1 bg-sky-500/10 text-sky-400 rounded-lg text-xs font-black uppercase tracking-widest border border-sky-500/20 shadow-sm group relative cursor-help">
               Real-time Conflict Checking Active
               <div className="absolute top-full left-0 mt-2 w-64 p-4 bg-slate-900 text-white rounded-2xl text-xs font-bold uppercase tracking-widest leading-relaxed opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none shadow-2xl z-50">
        ...
        <div className="flex items-center gap-4">
          <button 
            ...
            className="px-6 py-3 bg-slate-800 border border-slate-700 text-slate-200 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-700 transition-[transform,box-shadow] shadow-[3px_3px_0px_rgba(0,0,0,0.6)] hover:translate-x-[-1px] hover:translate-y-[-1px] hover:shadow-[4px_4px_0px_rgba(0,0,0,0.6)] active:translate-x-0 active:translate-y-0 active:shadow-[2px_2px_0px_rgba(0,0,0,0.6)] flex items-center gap-2"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
            {t('export_pdf')}
          </button>
          <div className="flex gap-2 bg-slate-900 p-1.5 rounded-[1.5rem] border border-slate-800 shadow-sm">
            {classes.map(c => (
              <button key={c.id} onClick={() => setSelectedClassId(c.id)} className={`px-5 py-2 rounded-[1rem] text-xs font-black uppercase tracking-widest transition-colors whitespace-nowrap ${selectedClassId === c.id ? 'bg-sky-500 text-white border border-sky-600 shadow-sm' : 'text-slate-450 hover:text-slate-200'}`}>{c.name}</button>
            ))}
          </div>
        </div>
      </header>
      ...
      {/* Demo Tip Card */}
      <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 no-print flex flex-col md:flex-row items-center gap-6 shadow-[6px_6px_0px_rgba(0,0,0,0.4)]">
         <div className="w-12 h-12 rounded-2xl bg-sky-500/10 border border-slate-800 flex items-center justify-center text-sky-400 shrink-0">
         ...
         <div className="flex-1 text-left">
            <p className="text-xs font-black text-white uppercase tracking-tighter">Interactive Learning</p>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1 leading-relaxed">Try dragging any slot below to a new period. Hold <span className="text-sky-400">ALT/Option</span> while dragging to CLONE instead of MOVE.</p>
         </div>
      </div>

      <div className="bg-slate-900/40 border border-slate-800 rounded-2xl overflow-hidden shadow-[10px_10px_0px_rgba(0,0,0,0.4)] max-w-full overflow-x-auto">
        <table className="w-full border-collapse table-fixed min-w-[1000px]">
          <thead>
            <tr className="bg-slate-900/60 border-b border-slate-800">
              <th className="border-r border-slate-800 p-6 text-xs font-black uppercase w-44 text-slate-300">{t('period')}</th>
              {days.map(d => <th key={d} className="border-r last:border-r-0 border-slate-800 p-6 text-xs font-black uppercase tracking-widest text-slate-300">{d}</th>)}
            </tr>
          </thead>
          <tbody>
            {Array.from({ length: totalPeriods }).map((_, pIdx) => {
              ...
              return (
                <tr key={pIdx} className="border-b border-slate-800 last:border-b-0">
                  <td className="border-r border-slate-800 p-6 text-center font-black text-slate-300 bg-slate-900/60 h-[120px]">
                    <div className="text-3xl tracking-tighter leading-none mb-1 text-white">{pIdx + 1}</div>
                    <div className="text-xs font-black text-slate-400 uppercase tracking-tighter">{pStart} — {pEnd}</div>
                  </td>
                  {Array.from({ length: 5 }).map((_, dIdx) => {
                    ...
                    if (lock) return (
                      <td key={dIdx} className="border-r last:border-r-0 border-slate-800 p-0 h-[120px] align-middle relative overflow-hidden bg-slate-950">
                        <div className="relative h-full flex flex-col items-center justify-center p-4 text-center">
                          <span className="text-xs font-black uppercase tracking-tight text-white leading-none">{lock.name}</span>
                          <span className="text-xs font-black text-slate-500 uppercase mt-1 tracking-widest">Locked Slot</span>
                        </div>
                      </td>
                    );
                    ...
                    return (
                      <td key={dIdx} 
                        ...
                        className={`border-r last:border-r-0 border-slate-800 p-0 h-[120px] transition-all relative align-top ${
                          isTarget && clashMsg ? 'bg-rose-950/40 ring-4 ring-rose-500 ring-inset z-50' : 
                          isTarget && !clashMsg ? 'bg-sky-500/10 ring-4 ring-sky-500 ring-inset z-50' : 
                          isSafeSlot ? 'bg-emerald-500/10 ring-2 ring-emerald-500/20 ring-inset' : 
                          'bg-slate-900/20 group hover:bg-slate-800/40'
                        }`}>
                        {slot ? (
                          <div className={`h-full flex flex-col relative transition-all cursor-grab active:cursor-grabbing ${isDraggingOrig ? 'opacity-20 scale-95 blur-[1px]' : ''} ${isTarget && !clashMsg ? 'scale-90 rotate-1' : ''}`} draggable="true" onDragStart={() => handleDragStart(dIdx, pIdx)}>
                            <div className="flex-1 flex flex-col items-center justify-center p-4 text-center overflow-hidden pointer-events-none">
                              <span className={`text-xs font-semibold leading-[1.1] line-clamp-2 tracking-tighter text-white`}>{getSubjectName(slot.subjectId)}</span>
                              {isTarget && !clashMsg && <span className="text-xs font-black text-sky-400 uppercase mt-1">Ready to Swap</span>}
                            </div>
                            <div className="h-8 flex items-center justify-center border-t border-slate-800 shrink-0 pointer-events-auto" style={{ backgroundColor: teacher?.color || '#cbd5e1' }}>
                              <span className="text-xs font-black text-slate-900 truncate px-4 tracking-tighter">{teacher?.name}</span>
                            </div>
                          </div>
                        ) : (
                          <div className={`h-full flex items-center justify-center relative ${isSafeSlot ? 'bg-emerald-50/10' : 'bg-[repeating-linear-gradient(45deg,transparent,transparent_6px,#fff_6px,#fff_7px)] opacity-[0.02]'}`}>
                             {isTarget && clashMsg && (
                               <div className="absolute inset-0 flex flex-col items-center justify-center bg-rose-500/20 p-2 text-center backdrop-blur-[2px]">
                                  ...
                                  <span className="text-xs font-black text-rose-300 uppercase tracking-tighter leading-none px-2">{clashMsg}</span>
                                </div>
                             )}
                          </div>
                        )}
