...
const ScheduleViewer: React.FC<ScheduleViewerProps> = ({ schedule, classes, teachers, subjects, profile, onGenerateRoadmap, onUpdateSlot, onNavigate }) => {
  ...
  if (!classes || classes.length === 0) {
    return <div className="p-20 text-center font-black text-slate-400 uppercase">Setup Required</div>;
  }
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-[3rem] p-10 w-full max-w-md shadow-2xl animate-fadeIn">
            ...
            <div className="space-y-3">
              <button onClick={() => handleApplyChange('')} className="w-full py-4 rounded-2xl bg-slate-50 text-slate-400 font-black text-[10px] uppercase mb-4">Clear Period</button>
              {currentClass.assignments.map(a => (
                <button key={a.subjectId} onClick={() => handleApplyChange(a.subjectId)} className="w-full py-5 rounded-2xl bg-white border border-slate-100 shadow-sm text-slate-900 font-black text-[11px] uppercase flex items-center justify-between px-6 hover:border-indigo-500 transition-all">
                  <span>{getSubjectName(a.subjectId)}</span>
                  <span className="text-[8px] text-slate-400">{getTeacherName(a.teacherId)}</span>
                </button>
              ))}
            </div>
            <button onClick={() => setEditingSlot(null)} className="mt-8 w-full text-slate-400 font-black text-[10px] uppercase">Close</button>
          </div>
        </div>
      )}

      <div className="flex justify-between items-center bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm overflow-x-auto gap-4">
        <div className="flex gap-2 min-w-max">
          {classes.map(c => (
            <button key={c.id} onClick={() => setSelectedClassId(c.id)} className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${selectedClassId === c.id ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-400'}`}>{c.name}</button>
          ))}
        </div>
      </div>
        ...
        <table className="w-full border-collapse min-w-[800px]">
          <thead>
            <tr className="bg-slate-50 border-b-[3px] border-slate-900">
              ...
              {days.map(d => <th key={d} className="border-r-[3px] last:border-r-0 border-slate-900 p-6 text-[11px] font-black uppercase">{d}</th>)}
            </tr>
          </thead>
          <tbody>
                  ...
                  if (lock) return (
                    <td key={dIdx} className="border-r-[3px] last:border-r-0 border-slate-900 p-0 h-36 bg-slate-100">
                      <div className="h-full flex items-center justify-center opacity-40 bg-[repeating-linear-gradient(45deg,transparent,transparent_10px,#000_10px,#000_11px)]">
                        <span className="text-[9px] font-black uppercase text-slate-900 bg-white px-3 py-1.5 rounded-lg shadow-sm border border-slate-900">{lock.name}</span>
                      </div>
                    </td>
                  );
                      ...
                      {slot ? (
                        <div className="h-full flex flex-col">
                          <div className="flex-1 flex flex-col items-center justify-center p-4 text-center">
                            <span className="text-[14px] font-black text-slate-900 uppercase leading-tight">{getSubjectName(slot.subjectId)}</span>
                          </div>
                          <div className="h-10 flex items-center justify-center border-t-[3px] border-slate-900" style={{ backgroundColor: teacher?.color || '#cbd5e1' }}>
                            <span className="text-[9px] font-black uppercase text-slate-900 truncate px-2">{teacher?.name}</span>
                          </div>
                        </div>
