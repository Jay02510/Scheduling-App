...
const CurriculumRoadmap: React.FC<CurriculumRoadmapProps> = ({ 
  ...
  const TextbookCard: React.FC<{ book: Textbook, onAssign?: () => void }> = ({ book, onAssign }) => {
    ...
    return (
      <div 
        className={`p-5 rounded-[2rem] border-[3px] border-slate-900 transition-all duration-300 group relative flex flex-col justify-between h-full ${
          isEditing ? 'bg-white shadow-[6px_6px_0px_rgba(99,102,241,1)] scale-[1.02] z-50' : 'bg-slate-50 hover:bg-white hover:shadow-[6px_6px_0px_rgba(15,23,42,1)] hover:translate-x-[-2px] hover:translate-y-[-2px]'
        }`}
      ...
      >
        <div className="flex justify-between items-start gap-4">
          <div className="flex-1">
            {isEditing ? (
              <div className="space-y-3">
                <input 
                  autoFocus
                  className="w-full bg-slate-100 border-2 border-slate-900 rounded-xl px-3 py-2 font-black text-slate-900 text-sm uppercase outline-none"
                  value={book.title}
                  onChange={(e) => updateBook(book.id, { title: e.target.value })}
                  onBlur={() => setEditingBookId(null)}
                />
                <button onClick={() => setEditingBookId(null)} className="w-full py-2 bg-indigo-600 border-2 border-slate-900 text-white rounded-xl text-[9px] font-black uppercase tracking-widest shadow-[3px_3px_0px_#000]">Save</button>
              </div>
            ) : (
          ...
          {!isEditing && (
            <button onClick={() => setEditingBookId(book.id)} className="p-2 text-slate-400 hover:text-indigo-650 transition-colors">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
            </button>
          )}
        </div>
        {onAssign && !isEditing && (
          <button onClick={onAssign} className="mt-4 w-full py-2 bg-[#0f172a] border-2 border-slate-900 text-white rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-indigo-600 hover:translate-y-[-1px] transition-all">Select</button>
        )}
      </div>
    );
  };
    ...
    <div className="space-y-10 animate-fadeIn pb-24 max-w-full">
      <header className="flex flex-col md:flex-row justify-between items-end gap-6 no-print">
        ...
        <div className="flex gap-4 items-center">
          <button onClick={() => setIsLibraryOpen(true)} className="px-6 py-4 bg-[#0f172a] text-white rounded-[1.5rem] font-black text-[10px] uppercase tracking-widest shadow-[4px_4px_0px_rgba(15,23,42,1)] flex items-center gap-2 border-[3px] border-slate-900 hover:translate-x-[-1px] hover:translate-y-[-1px] hover:shadow-[6px_6px_0px_rgba(15,23,42,1)] active:translate-x-0 active:translate-y-0 active:shadow-[2px_2px_0px_rgba(15,23,42,1)] transition-all">
          ...
          <div className="flex gap-2 bg-slate-100 p-1.5 rounded-[1.5rem] border-2 border-slate-900 shadow-sm">
            {classes.map(c => (
              <button key={c.id} onClick={() => setSelectedClassId(c.id)} className={`px-5 py-2 rounded-[1rem] text-[9px] font-black uppercase tracking-widest transition-all ${selectedClassId === c.id ? 'bg-[#0f172a] text-white border border-slate-900 shadow-sm' : 'text-slate-450 hover:text-slate-700'}`}>{c.name}</button>
            ))}
          </div>
        </div>
      </header>
      ...
      {selectingSlot && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[150] flex items-center justify-center p-6 no-print">
           <div className="bg-white rounded-[3rem] w-full max-w-2xl shadow-[10px_10px_0px_rgba(15,23,42,1)] overflow-hidden animate-fadeInUp border-[4px] border-slate-900 flex flex-col max-h-[85vh]">
              <div className="p-8 border-b-4 border-slate-900 bg-slate-50 flex justify-between items-center">
                 <div>
                    ...
                    <p className="text-[10px] font-black text-indigo-550 uppercase tracking-widest mt-1">Q{selectingSlot.quarterId + 1} Mapping • {currentClass.name}</p>
                 </div>
                 <button onClick={() => setSelectingSlot(null)} className="w-10 h-10 rounded-full border-2 border-transparent hover:border-slate-900 flex items-center justify-center transition-all bg-slate-100">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M6 18L18 6M6 6l12 12" /></svg>
                 </button>
              </div>
              <div className="p-8 space-y-4 overflow-y-auto custom-scrollbar flex-1 bg-slate-50/30">
                  <button onClick={() => handleAddBook(selectingSlot)} className="w-full py-8 border-[3px] border-dashed border-slate-900 rounded-[2.5rem] text-slate-400 font-black text-[10px] uppercase hover:text-indigo-600 bg-white transition-all shadow-[4px_4px_0px_rgba(15,23,42,1)] hover:shadow-[6px_6px_0px_rgba(15,23,42,1)] hover:translate-y-[-2px]">+ {t('register_resource')}</button>
      ...
      {isLibraryOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xl z-[100] flex items-center justify-end no-print">
           <div className="bg-white h-full w-full max-w-xl flex flex-col shadow-2xl animate-fadeIn border-l-[8px] border-slate-900">
              <div className="p-8 border-b-4 border-slate-900 bg-slate-50 flex justify-between items-center bg-slate-50">
                 ...
                 <button onClick={() => setIsLibraryOpen(false)} className="w-10 h-10 rounded-full border-2 border-transparent hover:border-slate-900 flex items-center justify-center bg-slate-100 transition-all"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M6 18L18 6M6 6l12 12" /></svg></button>
              </div>
              <div className="flex-1 overflow-y-auto p-8 space-y-4 custom-scrollbar">
                 <button onClick={() => handleAddBook()} className="w-full py-8 border-[3px] border-dashed border-slate-900 rounded-[2rem] text-slate-400 font-black text-[10px] uppercase hover:text-indigo-600 transition-all shadow-[4px_4px_0px_rgba(15,23,42,1)]">+ {t('register_resource')}</button>
      ...
      <div className="bg-white border-[4px] border-slate-900 rounded-[3rem] overflow-hidden shadow-[10px_10px_0px_rgba(15,23,42,1)] max-w-full overflow-x-auto">
