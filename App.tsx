...
const App: React.FC = () => {
  ...
  const handleMoveScheduleSlot = (source: { day: number, period: number }, target: { day: number, period: number }, classId: string, isCopy: boolean = false) => {
    ...
    if (sourceIdx > -1) {
      if (isCopy) {
        ...
        if (targetIdx > -1) newSlots[targetIdx] = clonedSlot;
        else newSlots.push(clonedSlot);
      } else {
        if (targetIdx > -1) {
  ...
  const handleFillScheduleSlots = (source: { day: number, period: number }, range: { startDay: number, endDay: number, startPeriod: number, endPeriod: number }, classId: string) => {
    ...
    for (let d = range.startDay; d <= range.endDay; d++) {
      for (let p = range.startPeriod; p <= range.endPeriod; p++) {
        ...
        if (targetIdx > -1) newSlots[targetIdx] = filledSlot;
  ...
  const handleMoveLockSlot = (source: { day: number, period: number }, target: { day: number, period: number }, isCopy: boolean = false) => {
    ...
    if (sourceIdx > -1) {
      if (isCopy) {
        ...
        if (targetIdx > -1) newLocks[targetIdx] = clonedLock;
        else newLocks.push(clonedLock);
      } else {
        if (targetIdx > -1) {
  ...
  const handleFillLockSlots = (source: { day: number, period: number }, range: { startDay: number, endDay: number, startPeriod: number, endPeriod: number }) => {
    ...
    for (let d = range.startDay; d <= range.endDay; d++) {
      for (let p = range.startPeriod; p <= range.endPeriod; p++) {
        ...
        if (targetIdx > -1) newLocks[targetIdx] = filledLock;
        else newLocks.push(filledLock);
      }
    }
    setLockedSlots(newLocks);
  };
  ...
  const handleGenerateMaster = async () => {
    ...
    try {
      ...
      setNotification({ msg: "Generation Complete: Institutional rhythms are synchronized.", type: 'success', action: () => { setActiveTab('homerooms'); setNotification(null); } });
      setIsLoading(false);
      setIsBackgrounded(false);
    } catch (e: any) {
    ...
    <Layout activeTab={activeTab} setActiveTab={setActiveTab}>
        ...
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[110] animate-fadeInUp w-full max-w-lg px-4">
          <div className={`p-6 rounded-[2.5rem] shadow-2xl flex items-center justify-between border ${notification.type === 'success' ? 'bg-[#0f172a] text-white border-white/10' : 'bg-rose-900 text-white border-rose-500/20'}`}>
            ...
            <div className="flex items-center gap-3">
              {notification.action && <button onClick={notification.action} className="px-5 py-2.5 bg-white text-slate-900 rounded-xl font-black text-[9px] uppercase tracking-widest hover:scale-105 transition-all">View Results</button>}
              <button onClick={() => setNotification(null)} className="p-2 text-slate-500 hover:text-white transition-colors"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M6 18L18 6M6 6l12 12" /></svg></button>
            </div>
          </div>
        </div>
      )}
      ...
      {isLoading && !isBackgrounded ? (
        <div className="flex flex-col items-center justify-center min-h-[80vh] animate-fadeIn space-y-12">
          ...
          <button onClick={() => { setIsBackgrounded(true); setActiveTab('home'); }} className="px-10 py-5 bg-white border border-slate-200 text-slate-900 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-50 transition-all shadow-sm">Run in Background</button>
        </div>
      ) : (
        <>
