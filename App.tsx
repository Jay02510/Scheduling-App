...
const App: React.FC = () => {
  ...
  // Tracking changed entities for incremental sync
  ...
  const markClassDirty = useCallback((classId: string) => {
    setDirtyClassIds(prev => new Set(prev).add(classId));
  }, []);

  const markTeacherClassesDirty = useCallback((teacherId: string) => {
    const affected = classes.filter(c => c.assignments.some(a => a.teacherId === teacherId)).map(c => c.id);
    setDirtyClassIds(prev => {
    ...
    setSchedule(prev => {
      ...
      for (let d = range.startDay; d <= range.endDay; d++) {
        for (let p = range.startPeriod; p <= range.endPeriod; p++) {
          ...
          const existing = newSlots.find(s => s.day === d && p === s.period && s.classId === classId);
  ...
  const handleGenerateMaster = async () => {
    ...
    try {
      ...
      const prevSlots = schedule?.weeklySlots || [];
    ...
    <Layout activeTab={activeTab} setActiveTab={setActiveTab} language={language} setLanguage={setLanguage}>
      <div className="mb-8 no-print flex flex-col md:flex-row gap-4 items-center justify-between bg-slate-100 p-4 rounded-[2rem] border border-slate-200">
        <div className="flex items-center gap-3">
          <div className={`w-3 h-3 rounded-full ${computeInputHash({ teachers, classes, lockedSlots, subjects, special: profile.specialInstructions || "", hours: profile.hours }) === lastInputHash ? 'bg-emerald-500' : 'bg-amber-500 animate-pulse'}`}></div>
          <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">
            {computeInputHash({ teachers, classes, lockedSlots, subjects, special: profile.specialInstructions || "", hours: profile.hours }) === lastInputHash 
              ? t('synchronized') 
              : dirtyClassIds.size > 0 
                ? `Incremental: ${dirtyClassIds.size} Pending Updates`
                : t('local_changes')}
          </span>
        </div>
        <div className="flex gap-4">
          <button onClick={() => { setForceFullSync(true); setDirtyClassIds(new Set()); handleGenerateMaster(); }} className="text-[8px] font-black uppercase tracking-widest text-slate-400 hover:text-indigo-600 transition-colors">Force Global Sync</button>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center min-h-[60vh] animate-fadeIn space-y-8">
          ...
          <div className="text-center max-w-sm">
            ...
            <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Optimizing Differential Streams...</p>
          </div>
        </div>
      ) : (
        <>
            ...
            <div className="mb-8 p-8 bg-rose-50 border-2 border-rose-200 rounded-[2.5rem] space-y-4 animate-fadeIn shadow-xl">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-rose-600 font-bold text-[10px] uppercase tracking-widest mt-0.5">{errorMessage || "Minor constraint conflicts detected."}</p>
                </div>
              </div>
              ...
              <div className="flex gap-4 items-center pt-2">
                ...
                <button onClick={handleGenerateMaster} className="text-[9px] font-black uppercase tracking-widest text-indigo-600 hover:text-indigo-800 transition-colors">Re-evaluate Partial Pass</button>
              </div>
            </div>
          )}
          ...
          {activeTab === 'setup' && <ScheduleForm profile={profile} setProfile={(p) => { setProfile(p); if(p.hours !== profile?.hours) setForceFullSync(true); }} teachers={teachers} setTeachers={(t) => { setTeachers(t); setForceFullSync(true); }} classes={classes} setClasses={(c) => { setClasses(c); setForceFullSync(true); }} textbooks={textbooks} setTextbooks={setTextbooks} lockedSlots={lockedSlots} setLockedSlots={(l) => { setLockedSlots(l); setForceFullSync(true); }} subjects={subjects} setSubjects={(s) => { setSubjects(s); setForceFullSync(true); }} onGenerate={handleGenerateMaster} schedule={schedule} onNavigate={setActiveTab} language={language} />}
